...
const getAlbums = asyncWrapper(async (req, res) => {
    const { sort, numericFilters, fields, title, artist } = req.query;

    const user = await User.findById(req.user["id"]);

    const queryObject = user.role === 'admin' ? {} : { owner: user._id }
    ...
    let result = Album.find(queryObject).populate('owner', { name: 1 });
...
const createAlbum = asyncWrapper(async (req, res, next) => {
    ...
    return res.status(StatusCodes.CREATED).json({ success: true, msg: album });
});
